...
public class LinphoneAddressImpl implements LinphoneAddress {
	...
	private native long newLinphoneAddressImpl(String uri,String displayName);//TODO 注册时最终调用这个本地方法，返回时一个long类型
	...
	protected LinphoneAddressImpl(String identity)  throws LinphoneCoreException{
		...
		Log.i("nativePtr", nativePtr+"");
		Log.i("info", this.getDisplayName()+","+this.getDomain()+","+this.getUserName()+","+this.getPort());
		if(nativePtr==0) {
