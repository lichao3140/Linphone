...
public class LinphoneActivity extends Activity implements OnClickListener, ContactPicked, ActivityCompat.OnRequestPermissionsResultCallback {
	...
	public void initAccount(){
		...
		Log.i("nbAccounts = "+nbAccounts);
		if(nbAccounts <= 0){
			AssistantActivity.instance().saveCreatedAccount("1076", "1076", "win10","120.25.126.228",null);
		}
		LinphoneCore lc = LinphoneManager.getLcIfManagerNotDestroyedOrNull();
		nbAccounts = LinphonePreferences.instance().getAccountCount();
		Log.i("nbAccounts = "+nbAccounts);
		if(nbAccounts > 0){
		    Log.i("lc.refreshRegisters()");
		    int accountIndex = 0;
		    LinphonePreferences.instance().setDefaultAccount(accountIndex);         
		    Log.i("accountPosition = "+0);
